...
class Scale:
    ...
    def __call__(self, imgmap):
        ...
        if isinstance(self.size, int):
            ...
            # if type(imgmap) == np.ndarray:
            #     # if imgmap.shape[1] == self.size[0]:
            #     return list(imgmap)
            #     # else:
            #     #     return [cv2.resize(i, self.size) for i in imgmap]
            # else:
...
class Normalize:
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):  # the mean and std are based on imagenet
        self.mean = mean
        self.std = std
    def __call__(self, imgmap):
