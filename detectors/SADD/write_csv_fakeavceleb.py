...
setattr(opt, 'real_dir_test', os.path.join(opt.out_dir, 'FakeAVCeleb_v1.2_inDFDCformat', 'test', 'pytmp', 'real'))
setattr(opt, 'fake_dir_test', os.path.join(opt.out_dir, 'FakeAVCeleb_v1.2_inDFDCformat', 'test', 'pytmp', 'fake'))
setattr(opt, 'real_dir_train', os.path.join(opt.out_dir, 'FakeAVCeleb_v1.2_inDFDCformat', 'train', 'pytmp', 'real'))
setattr(opt, 'fake_dir_train', os.path.join(opt.out_dir, 'FakeAVCeleb_v1.2_inDFDCformat', 'train', 'pytmp', 'fake'))
...
for i in range(len(fakeListTest)):
    for file in os.listdir(os.path.join(opt.fake_dir_test_balance, fakeListTest[i])):
        if os.path.isdir(os.path.join(opt.fake_dir_test_balance, fakeListTest[i], file)):
            audio_file = file + '.wav'
            test_set.append([os.path.join(opt.fake_dir_test_balance, fakeListTest[i], file),
                             os.path.join(opt.fake_dir_test_balance, fakeListTest[i], audio_file), 'fake'])

for i in range(len(realListTest)):
    for file in os.listdir(os.path.join(opt.real_dir_test_balance, realListTest[i])):
        if os.path.isdir(os.path.join(opt.real_dir_test_balance, realListTest[i], file)):
            audio_file = file + '.wav'
            test_set.append([os.path.join(opt.real_dir_test_balance, realListTest[i], file),
                             os.path.join(opt.real_dir_test_balance, realListTest[i], audio_file), 'real'])


write_list(train_set, os.path.join(opt.csv_root, 'train_fakeavceleb_split.csv'))
write_list(test_set, os.path.join(opt.csv_root, 'test_fakeavceleb_split.csv'))
