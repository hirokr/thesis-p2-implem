...
class TDMaskedConv1D(nn.Module):
    def __init__(
            self, 
            in_channels, 
            out_channels, 
    ...
    def forward(self, x, mask):
        B, C, T = x.size()
        [C_out, C_in, time_step_kernel_size] = self.conv.weight.shape
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            if kernel_size > 1:
                # Assuming kernel_size is odd and centered
                center = kernel_size // 2
                kernel_diff = self.conv.weight[:, :, center] - self.conv.weight[:, :, center-1]
                # kernel_diff = self.conv.weight[:, :, 0] + self.conv.weight[:, :, 2]
                # kernel_diff = self.conv.weight[:, :, 0] + self.conv.weight[:, :, 1] + self.conv.weight[:, :, 2]
                kernel_diff = kernel_diff[:, :, None]
                out_diff = F.conv1d(input=x, weight=kernel_diff, bias=self.conv.bias, stride=self.conv.stride,
                                    padding=0, dilation=self.conv.dilation, groups=self.conv.groups)
                
                kernel_diff1 = self.conv.weight[:, :, center+1] - self.conv.weight[:, :, center]
                kernel_diff1 = kernel_diff1[:, :, None]
                out_diff1 = F.conv1d(input=x, weight=kernel_diff1, bias=self.conv.bias, stride=self.conv.stride,
                                    padding=0, dilation=self.conv.dilation, groups=self.conv.groups)
                
                out = out_normal - self.theta * (out_diff+out_diff1)
...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # conv
        x= x.type(torch.cuda.FloatTensor)
        # print("1. Input x contains Inf:", torch.isinf(x).any().item())
        out_conv = self.conv(x)
        # print("2. Input out_conv contains Inf:", torch.isinf(out_conv).any().item())
        # compute the mask
        if self.stride > 1:
            # downsample the mask using nearest neighbor
            out_mask = F.interpolate(
...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        if with_Difference:
                self.query_conv = TDMaskedConv1D(
        ...
        if with_Difference:
            self.value_conv = TDMaskedConv1D(
                self.n_embd, self.n_embd, kernel_size,
                stride=stride, padding=padding, groups=self.n_embd, bias=False
            )
        else:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_lstm:
            self.lstm = nn.LSTM(input_size=n_embd, hidden_size=int(n_embd/2), num_layers=1, batch_first=True, bidirectional=True)
    ...
    def forward(self, x, mask, pos_embd=None):
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
            ...
            # print("lstm")
        if self.use_lstm:
            hidden_state , _ = self.lstm(x.transpose(1,2))
