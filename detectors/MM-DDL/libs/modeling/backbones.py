...
from .blocks import (get_sinusoid_encoding, TransformerBlock, MaskedConv1D,TDMaskedConv1D,
                     ConvBlock, LayerNorm)
# from .sincconv import RawNet
    ...
    def __init__(
        ...
        if isinstance(n_in, (list, tuple)):
            ...
            if with_Difference:
                ...
                TDMaskedConv1D(c0, c1, 1) for c0, c1 in zip(n_in, n_embd)
            ])
            else:
                self.proj = nn.ModuleList([
...
# @register_backbone("sincconv")
# class SincConvBackbone(nn.Module):
#     """
#         A backbone that with only conv
#     """
#     def __init__(
