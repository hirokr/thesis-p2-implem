...
def extract_label_dict(self):
        ...
        for i in range(0, len(lines), 2):
            methods = eval(lines[i + 1].strip())[-1]
            for method in methods:
                if method not in label_dict:
                    label_dict[method]=current_label
                    current_label += 1
        
        return label_dict


def load_gt_seg_from_partial_label(label_file, split=None, label='label_id', label_offset=0, label_dict=None):
    # load label file
    with open(label_file, "r", encoding="utf8") as f:
        lines = f.readlines()

    vids, starts, stops, all_labels = [], [], [], []
    for i in range(0, len(lines), 1):
        ...
        if record[2]=='true':
            ...
            array_parts = record[3:]
            array_str = ' '.join(array_parts)
            intervals_str = array_str.strip('[]').split('], [')
            intervals = [list(map(float, interval.split(', '))) for interval in intervals_str]
            segments = np.array(intervals).reshape(-1, 2)
...
def load_gt_seg_from_label(label_file, split=None, label='label_id', label_offset=0, label_dict=None):
    # load label file
    with open(label_file, "r", encoding="utf8") as f:
        lines = f.readlines()
    ...
    for i in range(0, len(lines), 2):
        ...
        if label[1]=='True':
            ...
            # get segments information
            segments = np.array(label[2]).reshape(-1, 2)
            segments = np.asarray(segments, dtype=np.float32)
...
def load_gt_seg_from_add_label(label_file, split=None, label='label_id', label_offset=0, label_dict=None):
    # load label file
    with open(label_file, "r", encoding="utf8") as f:
        lines = f.readlines()
    ...
    for i in range(0, len(lines), 1):
        ...
        if record[-1]=='1':
            duration = float((record[1].split("-"))[1])
            continue
        else:
            segments = []
            ...
            for split in splits:
                if split.split("-")[-1]=="F":
                    segments.append([split.split("-")[0], split.split("-")[1]])
                    segments = np.array(segments).reshape(-1, 2)
                    segments = np.asarray(segments, dtype=np.float32)
            duration = float((record[1].split("-"))[-2])
                # get label form dict
            labels = [0]
            labels = (np.asarray(labels, dtype=np.int64))
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.split = split
        if self.dataset_name == "add_dev.txt":
            self.ground_truth = load_gt_seg_from_add_label(
                ant_file, split=self.split, label=label, label_offset=label_offset, label_dict=self.label_dict)
        if self.dataset_name == 'dev.txt' or self.dataset_name == 'dev_location.txt' or self.dataset_name == 'eval.txt':
            self.ground_truth = load_gt_seg_from_label(
                ant_file, split=self.split, label=label, label_offset=label_offset, label_dict=self.label_dict)
            print(self.label_dict)

        if self.dataset_name == 'label_dev_partial.txt' or self.dataset_name == 'label_eval_partial.txt':
            self.ground_truth = load_gt_seg_from_partial_label(
                ant_file, split=self.split, label=label, label_offset=label_offset, label_dict=self.label_dict)
        ...
        print("activte",self.activity_index)
        # pass
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        # print("ground_truth:",ground_truth_by_label.get_group(0).reset_index(drop=True),)
...
def compute_average_precision_detection(
    ...
    # print("tp:",tp_cumsum)
    # print("fp:",fp_cumsum)
    # print("pre:",precision_cumsum)
    # print("recall:",recall_cumsum)
    # print("---------------------")
...
# def interpolated_prec_rec(prec, rec):
#     """Interpolated AP - VOCdevkit from VOC 2011, with PR curve plotting.
#     """
#     mprec = np.hstack([[0], prec, [0]])
#     mrec = np.hstack([[0], rec, [1]])
#     for i in range(len(mprec) - 1)[::-1]:
...
#     # Plotting PR curve
#     plt.figure()
#     plt.plot(mrec, mprec, marker='.')
#     plt.title('Precision-Recall Curve')
#     plt.xlabel('Recall')
#     plt.ylabel('Precision')
#     plt.grid(True)

#     plt.savefig("P-R_curve.png")  # Save the figure
#     plt.close()
#     print("P-R Curve saved!")
#     return ap
