...
from ..modeling import MaskedConv1D, Scale, AffineDropPath, LayerNorm,sincconv
# from ..modeling.sincconv import SincConv
...
def make_optimizer(model, optimizer_config):
    ...
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm,torch.nn.BatchNorm1d,torch.nn.LSTM)

    # loop over all modules / params
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            fpn = '%s.%s' % (mn, pn) if mn else pn # full param name
            if pn.endswith('bias') or pn.endswith('bias_hh_l0'):
                ...
                no_decay.add(fpn)
            elif mn.endswith("lstm"):
                no_decay.add(fpn)
            elif pn == 'in_proj_weight' and isinstance(m, torch.nn.MultiheadAttention):
                decay.add(fpn)
